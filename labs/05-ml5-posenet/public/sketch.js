 ...
 * This algorithm classifies input in "k" classes.
 * 
 * see: https://ml5js.org/docs/KNNClassifier
 */
...
function gotResults(err, result) {
  ...
  if (result.confidencesByLabel) {
    ...
    if (result.label) {
      ...
      let roundedConf = Math.round(confidences[result.label] * 100);
      select('#confidence').html(`${roundedConf} %`);
    }

    let roundedConfA = Math.round(confidences['A'] * 100);
    let roundedConfB = Math.round(confidences['B'] * 100);
    select('#confidenceA').html(`${confidences['A'] ? roundedConfA : 0} %`);
    select('#confidenceB').html(`${confidences['B'] ? roundedConfB : 0} %`);
  }
